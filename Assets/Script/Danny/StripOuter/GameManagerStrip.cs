...
public class GameManagerStrip : MonoBehaviour
{
    public static GameManagerStrip instance;
    ...
    private string[] mission = new string[] { "Strip 2 TSS-802-D02 cable ", "Strip 4 101518GR cable 4mm | 20mm ", "Congrate!\nYou Unclock Next Level!" };
    private string[] cableQtyRequire = new string[] { "/2", "/4", "" };
    private string[] textError = new string[] { "Touch strip threshold", "This already strip!", "Length is wrong! Should be 20mm!", "Length is wrong! Should be 4mm!", "Length is wrong! Should be 20mm or 4mm!" };
    ...
    public void LengthShow()
    {
        if (missionComplete == 1)
        {
            if (GRCable.transform.position.x >= 21f)
            {
                ...
                if (gameDistance >= 3f && gameDistance < 6f)
                {
                    gameDistance = 4f;
                    lengthAnimator.SetInteger("correct", 1);
                }

                else if (gameDistance >= 19f && gameDistance < 22f)
                {
                    gameDistance = 20f;
                    lengthAnimator.SetInteger("correct", 1);
                }
    ...
    public void Mission()
    {
        ...
        if (missionComplete == 0 && isTSS)
        {
            ...
            TSSCable.GetComponent<Rigidbody>().isKinematic = true;
    ...
    IEnumerator MissionDone()
    {
        ...
        yield return new WaitForSeconds(1f);
        if (missionComplete == 0 && noCompleteCount == 2)
        {
            noCompleteCount = 0;
            missionComplete++;
        ...
        if (missionComplete == 1 && noCompleteCount == 4)
        {
            noCompleteCount = 0;
            missionComplete++;
        }
